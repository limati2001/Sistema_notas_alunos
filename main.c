...
static void listar_alunos(const Aluno alunos[], int total) {
    ...
    printf("\n=== LISTA ===\n");
    printf("%-6s | %-20s | %5s | %5s | %5s\n", "MAT", "NOME", "N1", "N2", "MED");
    printf("-----------------------------------------------------------\n");

    for (int i = 0; i < total; i++) {
        printf("%-6d | %-20s | %5.1f | %5.1f | %5.1f\n",
               alunos[i].matricula,
               alunos[i].nome,
               alunos[i].n1,
               alunos[i].n2,
...
static void editar_notas(Aluno alunos[], int total) {
    ...
    printf("Nota 1 atual: %.1f\n", alunos[idx].n1);
    printf("Nova Nota 1: ");
    if (scanf("%f", &alunos[idx].n1) != 1) { printf("Entrada invalida.\n"); limpar_buffer(); return; }

    printf("Nota 2 atual: %.1f\n", alunos[idx].n2);
    printf("Nova Nota 2: ");
    if (scanf("%f", &alunos[idx].n2) != 1) { printf("Entrada invalida.\n"); limpar_buffer(); return; }
...
int main(void) {
    ...
    while (1) {
        ...
        if (opcao == 1) {
            ...
            if (salvar(alunos, total)) printf("Salvo em %s.\n", ARQ);
            else printf("Erro ao salvar.\n");
        } else if (opcao == 0) {
            if (!salvar(alunos, total)) printf("Erro ao salvar.\n");
            else printf("Salvo em %s. Tchau!\n", ARQ);
            break;
        } else {
